...
final class ViewController: UIViewController {
    ...
    private func configureDataSource() -> UICollectionViewDiffableDataSource<Section, Animal> {
        let cellRegistration = UICollectionView.CellRegistration<UICollectionViewListCell, Animal> { cell, indexPath, itemIdentifier in
            ...
            let customAccessory = UICellAccessory.CustomViewConfiguration(customView: UIImageView(image: UIImage(systemName: "pencil")), placement:
                    . trailing(displayed: .always, at: { accessories in
                        return 1 }), reservedLayoutWidth: .actual, tintColor: .blue, maintainsFixedSize: false)
            
            cell.accessories = [.disclosureIndicator(displayed: .always, options: .init(reservedLayoutWidth: .actual, tintColor: .blue)),
            .customView(configuration:customAccessory)]
        }
